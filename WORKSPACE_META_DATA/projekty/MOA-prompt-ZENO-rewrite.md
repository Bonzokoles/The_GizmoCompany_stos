## Prompt dla MOA — przepisz artykuł ZENO Browser w przystępnym języku

**Zadanie:** Przepisz poniższy artykuł techniczny o ZENO Browser tak, żeby był zrozumiały dla każdego — właściciela firmy, menedżera, kogoś, kto słyszał o AI, ale nie jest programistą. Nie zmniejszaj liczby sekcji ani nie pomijaj żadnych informacji. Tylko zamień styl.

---

### Zasady, których bezwzględnie przestrzegaj:

1. **Pisz jak do właściciela firmy, nie jak do programisty.**
   Zamiast „warstwa preload z contextBridge i IPC handlers" napisz „bezpieczna przepustnica, która kontroluje co może robić przeglądarka i co może przejść do kolejnego etapu".

2. **Żadnych nazw technicznych bez wyjaśnienia w prostym zdaniu.**
   Jeśli musisz użyć słowa technicznego, od razu wytłumacz je w nawiasie lub w kolejnym zdaniu — tak jak tłumaczysz coś sąsiadowi.

3. **Używaj konkretnych przykładów z życia firmy.**
   Zamiast „narzędzie do analizy danych pipeline" napisz coś w stylu: „To jak mieć na stałe zatrudnionego analityka, który każdego ranka przygotowuje skrót tego, co wydarzyło się wczoraj w firmie."

4. **Nigdy nie ujawniaj nazw firm klientów.** Zamiast tego pisz „firma z branży e-commerce", „przedsiębiorstwo budowlane", „firma usługowa".

5. **Sekcje i nagłówki: ludzkie nazwy, nie kody.**
   Zamiast: `### 1. filesystem-tools — operacje na plikach lokalnych`
   Napisz: `### 1. Dostęp do plików i dokumentów`

6. **Opisuj KORZYŚĆ, nie mechanizm.**
   Zamiast: „moduł ten umożliwia odczyt i zapis plików przez MCP API"
   Napisz: „Dzięki temu narzędziu ZENO może przeszukać Twoje dokumenty, faktury czy dane firmowe — bez wysyłania ich gdziekolwiek na zewnątrz."

7. **Prywatność danych — wyjaśnij metaforą.**
   Użyj porównania: „To jak wysłanie komuś podsumowania spotkania zamiast pełnego nagrania — zachowujesz to, co ważne dla siebie, a na zewnątrz trafia tylko efekt, nie dane."

8. **Styl: ciepły, bezpośredni, konkretny.**
   Nie używaj słów: „implementacja", „noda", „endpoint", „handler", „callback", „runtime", „daemon", „sandboxing". Każde takie słowo zastąp opisem tego, co ono robi w praktyce.

9. **Długość:** Nie skracaj — raczej dodawaj zdania wyjaśniające. Artykuł ma być kompletny i użyteczny, nie skrócony.

10. **Wzorzec tonu:** Sekcja o czwartej warstwie (mybonzo.com) jest napisana dokładnie w takim stylu, jaki chcemy. Użyj jej jako wzorca dla całego tekstu.

---

### Wzorzec dobrego tonu (fragment sekcji o mybonzo.com):

> Mybonzo.com dysponuje własną siatką asystentów AI, którzy znają się na prowadzeniu stanów magazynowych, obsłudze klienta, bazach dostawców, usługach i planowanych projektach. Możemy korzystać z generatorów reklam, maili i wpisów do mediów społecznościowych, prognozować trendy rynkowe, oceniać sposób działania firmy i dowiadywać się rzeczy, do których znalezienia normalnie potrzebna byłaby wykwalifikowana osoba z dużym doświadczeniem.
>
> Innymi słowy: zamiast zatrudniać analityka, kontrolera, specjalistę od marketingu i doradcę finansowego — masz ich wszystkich pod jednym dachem, dostępnych w każdej chwili.

---

### Wzorzec złego tonu (jak NIE pisać):

> ZENO Browser Shell to wielowarstwowa architektura Electron z izolacją kontekstów przez contextBridge, IPC handlers w warstwie Main Process oraz sandboxowanym rendererem. Warstwa preload eksponuje selektywnie API poprzez ipcRenderer, co zapewnia minimalne attack surface przy zachowaniu pełnej funkcjonalności MCP toolset.
...
- Warstwa UI (Shell): BrowserShellLayout.tsx, WorkspaceHost.tsx, PanelGroups, PanelFallback, BrowserUI, AppFolderPanel; motyw Deep Dark. Kluczową rolą jest opakowanie UI, zarządzanie закładками i organizacja paneli oraz wyglądu aplikacji.
...
**Po przepisaniu:** Sprawdź każdy akapit zadając sobie pytanie: „Czy moja mama, która prowadzi sklep, zrozumiałaby to zdanie?" Jeśli nie — przepisz jeszcze raz.
