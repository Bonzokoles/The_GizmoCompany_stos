...
This project is indexed by GitNexus as **WWW_Zen_BRo_wser_org3** (34519 symbols, 83002 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/WWW_Zen_BRo_wser_org3/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/WWW_Zen_BRo_wser_org3/context` | Codebase overview, check index freshness |
| `gitnexus://repo/WWW_Zen_BRo_wser_org3/clusters` | All functional areas |
| `gitnexus://repo/WWW_Zen_BRo_wser_org3/processes` | All execution flows |
| `gitnexus://repo/WWW_Zen_BRo_wser_org3/process/{name}` | Step-by-step execution trace |
...
<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1777075430309-dzhea7 | 2026-04-25T00:04:00.718Z

**Codebase Map:**
  src/: App, ElectronApp, main, vite-env.d
  src/__tests__/: marketplace-service.test, plugin-manager.test, setup
  src/components/: ErrorBoundary, LoadingSpinner, AgentsCreatorPanel, AgentWorkspacePanel, AIGatewayPanel, AIPanel, AnalyticsPanel, SecurityMonitor
  src/data/: promptLibrary
  src/hooks/: useJimboKitStore
  src/plugin-system/: plugin-api, plugin-loader, plugin-manager, plugin-registry, capability-types, manifest-validator, permission-gate, policy-engine, auto-updater, marketplace-service
  src/services/: gateway, deepseek, edenai, openrouter, moa-tools-checker
  src/shared/: channels, ai.contracts, browser.contracts, hub.contracts, plugin.contracts
  src/types/: electron.d, security-constants
  src/vendor/: monaco-setup-stub, monaco-setup, shiki-streamdown-lite
  scripts/: _fix2_datasets_grid, _fix_datasets_js, cf-process-datasets, check-moa-tools, copilot-sdk-smoke-test, create-installer-assets, fetch-top-rated, generate-crawler-report, generate-mascot-manifest, genera...
