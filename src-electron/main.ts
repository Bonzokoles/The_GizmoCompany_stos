...
import {
  AgentsCreatorService,
  type AgentWorkspace,
  type PromptSnippet,
} from "./services/agents-creator-service";
...
function createWindow() {
  ...
  mainWindow.webContents.session.webRequest.onHeadersReceived(
    (details, callback) => {
      ...
      const csp = isDev
        ? "default-src 'self' 'unsafe-inline' 'unsafe-eval' ws://localhost:* http://localhost:*; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://analytics.mybonzo.com https://plausible.mybonzo.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' ws://localhost:* http://localhost:* https:; frame-src *;"
        : "default-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' https://analytics.mybonzo.com https://plausible.mybonzo.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' ws://localhost:* http://localhost:* https:; frame-src *;";
...
async function initializeServices() {
  try {
    ...
    knowledgeHubService
      .autoRegisterHubLibraries()
      .then((r) =>
        console.log(
          `✅ Knowledge Hub: zarejestrowano ${r.registered} bibl. (pominięto: ${r.skipped})`,
        ),
      )
      .catch(() =>
        console.log(
          "⚠️ Knowledge Hub: auto-register failed (The_DEVz_HUB_of_work may not exist)",
        ),
      );
    ...
    meilisearchService.ensureIndexes().catch(() => {
      /* Meili may not be running yet */
    });
...
function startJimboHub() {
  ...
  const spawnHub = () => {
    ...
    testServer.once("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE") {
        console.log(
          "✅ JIMBO Hub already running on :4224 (started externally) — skipping spawn",
        );
        return;
      }
      doSpawn();
  ...
  const doSpawn = () => {
    ...
    jimboHubProcess.on("exit", (code, signal) => {
      ...
      console.warn(
        `⚠️ JIMBO Hub exited (code=${code}, signal=${signal}) — restarting in 5s`,
      );
...
function setupIPCHandlers() {
  ...
  // AI Gateway
  ipcMain.handle("ai:execute", async (_, request: AIRequest) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (
      _,
      opts?: { fromTabId?: string; toTabId?: string; limit?: number },
    ) => {
  ...
  ipcMain.handle(
    "tabs:share-session",
    async (_, fromTabId: string, toTabId: string, keys: string[]) => {
  ...
  ipcMain.handle(
    "tabs:share-auth",
    async (_, fromTabId: string, toTabId?: string) => {
  ...
  ipcMain.handle(
    "workflow:create",
    async (_, id: string, steps: WorkflowStep[]) => {
  ...
  ipcMain.handle(
    ...
    async (_, command: string, cwd?: string) => {
      if (!isValidString(command))
        return { success: false, error: "Nieprawidłowe polecenie" };
      if (isBlockedCommand(command)) {
        return {
          success: false,
          error: "Polecenie zablokowane ze względów bezpieczeństwa",
        };
      }
  ...
  ipcMain.handle("terminal:set-cwd", async (_, dir: string) => {
    if (!isValidString(dir))
      return { success: false, error: "Nieprawidłowa ścieżka" };
    try {
  ...
  ipcMain.handle("terminal:kill-process", async (_, pid: number) => {
    if (typeof pid !== "number" || pid <= 0)
      return { success: false, error: "Nieprawidłowy PID" };
    try {
  ...
  ipcMain.handle(
    "copilot:run-prompt",
    async (_, request: { prompt: string; model?: string; cwd?: string }) => {
  ...
  ipcMain.handle(
    "search:web",
    async (_, query: string, filters?: SearchFilters) => {
  ...
  ipcMain.handle(
    "search:local",
    async (_, query: string, libraryId?: string) => {
      if (!isValidString(query))
        return { success: false, error: "Nieprawidłowe zapytanie" };
      try {
  ...
  ipcMain.handle(
    "websurfx:search",
    async (_, query: string, filters?: Record<string, unknown>) => {
      if (!isValidString(query))
        return { success: false, error: "Brak zapytania" };
      try {
        const result = await websurfxService.search(
          query,
  ...
  ipcMain.handle(
    "sist2:search",
    async (_, query: string, size?: number, from?: number) => {
      if (!isValidString(query))
        return { success: false, error: "Brak zapytania" };
      try {
  ...
  ipcMain.handle(
    "catalog:add-library",
    async (_, name: string, rootPath: string, extensions?: string[]) => {
      if (!isValidString(name) || !isValidString(rootPath))
        return { success: false, error: "Nieprawidłowe parametry" };
      try {
  ...
  ipcMain.handle("catalog:index-library", async (_, libraryId: string) => {
    if (!isValidString(libraryId))
      return { success: false, error: "Nieprawidłowe ID" };
    try {
  ...
  ipcMain.handle("catalog:read-file", async (_, filePath: string) => {
    ...
    try {
      ...
      if (!file)
        return {
          success: false,
          error: "Plik niedostępny lub poza biblioteką",
        };
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!config || !isValidString(config.name))
        return { success: false, error: "Nieprawidłowa nazwa" };
      try {
  ...
  ipcMain.handle(
    "ac:update-workspace",
    async (_, agentId: string, updates: Partial<AgentWorkspace>) => {
      if (!isValidString(agentId))
        return { success: false, error: "Nieprawidłowe ID" };
      try {
  ...
  ipcMain.handle(
    "ac:add-kb-file",
    async (_, agentId: string, sourcePath: string) => {
      if (!isValidString(agentId) || !isValidString(sourcePath))
  ...
  ipcMain.handle(
    "ac:import-from-topic",
    async (_, agentId: string, topicId: string) => {
      if (!isValidString(agentId) || !isValidString(topicId))
        return { success: false, error: "Brak danych" };
      try {
  ...
  ipcMain.handle(
    "ac:import-from-url",
    async (_, agentId: string, url: string) => {
      if (!isValidString(agentId) || !isValidString(url))
        return { success: false, error: "Brak danych" };
      try {
  ...
  ipcMain.handle(
    "ac:remove-kb-file",
    async (_, agentId: string, fileName: string) => {
      if (!isValidString(agentId) || !isValidString(fileName))
        return { success: false, error: "Brak danych" };
      try {
  ...
  ipcMain.handle(
    "ac:add-prompt-snippet",
    async (_, agentId: string, snippet: PromptSnippet) => {
      if (!isValidString(agentId) || !snippet?.name)
        return { success: false, error: "Brak danych" };
      try {
  ...
  ipcMain.handle(
    "ac:search-rag",
    async (_, agentId: string, query: string, limit?: number) => {
  ...
  ipcMain.handle("dialog:read-dir-files", async (_, dirPath: string) => {
    ...
    function walk(dir: string) {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          ALLOWED.includes(nodePath.extname(entry.name).toLowerCase())
        ) {
          try {
    ...
    try {
      ...
      return { success: false, files: [], error: getErrorMessage(e) };
    }
  });
}
...
app.on("ready", async () => {
  try {
    ...
    mcpServer.startHTTP(3847).catch((err: Error) => {
      console.warn(
        "⚠️ MCP HTTP server failed to start (non-fatal):",
        err.message,
      );
    });
  } catch (error) {
