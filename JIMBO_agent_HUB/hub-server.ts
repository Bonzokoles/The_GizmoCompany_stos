...
import {
  listGooseSessions,
  importGooseSession,
} from "./agents/goose-session-importer.js";
import {
  listCFProjects,
  listDeployments,
  triggerDeploy,
...
function saveSessionState() {
  ...
  try {
    ...
    console.warn("[HUB] Nie udało się zapisać stanu sesji:", e);
  }
}
...
if (savedSession) {
  ...
  console.log(
    `[HUB] Wznowiono sesję Goose: ${savedSession.sessionName} (${savedSession.taskCount} tasków)`,
  );
} else {
...
Wykonaj zadanie jeszcze raz uwzględniając powyższą wskazówkę.`;
          ...
          console.log(
            `[AutoRetry] task=${originalTaskId} retry=${retryNum} score=${reflexionResult.score.toFixed(2)}`,
          );
          goose
            .runTask({ id: retryTaskId, instructions: improvedInstruction })
            .catch((err) => {
  ...
  if (instructions && result.output && result.exitCode === 0) {
    try {
      ...
      const synthesisMessages: Message[] = [
        {
          role: "system",
          content: SYSTEM_PROMPT.content + memory.formatCoreForPrompt(),
        },
      ...
      if (summaryContent.trim()) {
        broadcast(result.taskId, {
          type: "goose:synthesis",
          taskId: result.taskId,
...
function selectModel(
  ...
  if (
    /kod|debug|bł[aą]d|error|typescript|tsx?|fix|komponent|refaktor|plik|import/.test(
      q,
    )
  )
...
app.post("/chat", async (req, res) => {
  ...
  if (!messages?.length) {
    return res
      .status(400)
      .json({ error: "Brak wiadomości (pole: messages lub message)" });
  }
  ...
  const [skillsPrefix, recallMatches] = await Promise.all([
    ...
    lastUserText.trim()
      ? memory.searchRecall(lastUserText, 3)
      : Promise.resolve([]),
  ]);
  ...
  const filePathPattern =
    /(?:[A-Za-z]:\\[\w\\\s./-]+|\/[\w/\s.:-]+|[\w./\\-]+\.(?:csv|xlsx|json|md|ts|tsx|js|py|txt|pdf|log|yaml|yml))/gi;
  const mentionedPaths = lastUserText.match(filePathPattern) ?? [];
  let fileCatalogContext = "";
  if (mentionedPaths.length > 0) {
    const catalog = memory
      .listArchival(100)
      .filter((e) => e.source === "file_catalog");
    const relevant = catalog.filter((e) =>
      mentionedPaths.some((p) =>
        e.content.toLowerCase().includes(p.toLowerCase()),
      ),
    );
  ...
  try {
    if (stream) {
      ...
      try {
        ...
        // Fallback: model nie obsługuje streaming lub zwrócił tool_calls — użyj chat()
        console.warn("[HUB /chat] Stream failed, fallback to chat():", (streamErr as Error).message);
        if (!assistantText) {
          const fallback = await chat(llm, fullMessages, undefined, selectedModel);
      ...
      if (assistantText.trim())
        memory.saveRecall("assistant", assistantText, sessionId);
    } else {
      ...
      if (gooseMatch && goose.isAvailable()) {
        ...
        res.json({
          ...
          content,
          usage: result.usage,
          gooseDispatched: true,
          gooseTaskId: taskId,
          gooseInstruction: instruction,
        });
        if (content.trim()) memory.saveRecall("assistant", content, sessionId);
        goose
          .runTask({ id: taskId, instructions: instruction })
          .catch((err) => {
            console.error(
              `[HUB] Auto-dispatch task ${taskId} błąd:`,
              err.message,
            );
          });
      } else {
...
app.post("/agent/run", async (req, res) => {
  ...
  if (!instruction) {
    return res
      .status(400)
      .json({ error: "Brak instrukcji (pole: instructions lub task)" });
  }
...
app.post("/files/register", async (req, res) => {
  ...
  const id = await memory.saveArchival(content, "file_catalog", [
    "file",
    "catalog",
    ...tags,
  ]);
...
app.post("/files/analyze", async (req, res) => {
  ...
  if (!goose.isAvailable())
    return res.status(503).json({ error: "Goose niedostępny" });
...
Koniec outputu oznacz: --- KONIEC ANALIZY ---`;
    ...
    const llmCompletion = await chat(llm, [
      {
        role: "system",
        content:
          "Jesteś analitykiem plików. Na podstawie outputu Goose stwórz strukturalny raport w formacie JSON.",
      },
      {
        role: "user",
  ...
  if (autoRegister && report.summary) {
    ...
    await memory.saveArchival(content, "file_catalog", [
      "file",
      "catalog",
      ...report.tags,
    ]);
  ...
  const isValuable =
    ...
    !["package-lock.json", ".env", "yarn.lock"].includes(
...
app.post("/files/scan", async (req, res) => {
  ...
  function scanDir(
    dirPath: string,
    currentDepth: number,
  ): Array<{ path: string; type: "file" | "dir"; ext: string }> {
  ...
  const listText = toDescribe
    .map(
      (e) =>
        `${e.type === "dir" ? "[DIR]" : "[FILE]"} ${path.basename(e.path)}`,
    )
    .join("\n");
    ...
    const scanCompletion = await chat(llm, [
      {
        role: "system",
        content:
          'You are a code analyst. Return ONLY a JSON object mapping each filename to a short description (max 15 words). Format: {"filename.ts": "description", ...}',
      },
      {
        role: "user",
...
app.post("/skills/save", async (req, res) => {
  ...
  if (!name?.trim() || !description?.trim() || !code?.trim()) {
    return res
      .status(400)
      .json({ error: "Wymagane pola: name, description, code" });
  }
  try {
...
app.get("/skills/export", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="skills-export-${Date.now()}.json"`,
  );
  res.json(payload);
});
...
app.post("/skills/import", async (req, res) => {
  ...
  for (const raw of body.skills) {
    ...
    if (!s.name || !s.description || !s.code) {
      skipped.push(String(s.name ?? "?"));
      continue;
    }
    // Sprawdź duplikat
    const existing = skills
      .list(s.namespace as string | undefined)
      .find(
        (e) => e.name === s.name && e.namespace === (s.namespace ?? "global"),
      );
    if (existing) {
      skipped.push(String(s.name));
      continue;
...
app.get("/skills/graph", (req, res) => {
  ...
  const withEmbed = rows.map((r) => {
    ...
    let emb: number[] = [];
    try {
      emb = JSON.parse((fullSkill as any)?.embedding ?? "[]");
    } catch {
      /* ignore */
    }
    return { ...r, emb, tags: Array.isArray(r.tags) ? r.tags : JSON.parse((r.tags as string) ?? "[]") };
  });
    ...
    for (let j = i + 1; j < withEmbed.length; j++) {
      ...
      if (sim >= threshold) {
        edges.push({
          ...
          animated: sim > 0.85,
          style: {
            stroke: `rgba(148,163,184,${(sim - threshold) * 2})`,
            strokeWidth: Math.round(sim * 3),
          },
          label: sim.toFixed(2),
          data: { similarity: sim },
        });
...
app.post("/skills/import", async (req, res) => {
  const { skills: incoming, mode } = req.body as {
    skills?: unknown[];
    mode?: string;
  };
  ...
  const result = await skills.importJson(
    incoming as Partial<import("./skills/skill-manager.js").Skill>[],
    importMode,
  );
...
app.get("/skills/export-skill-md", (req, res) => {
  ...
  if (save) {
    ...
    try {
      ...
      console.log(
        `[HUB] SKILL.md zapisany: ${skillMdPath} (${md.length} znaków)`,
      );
    } catch (err) {
      console.warn(
        "[HUB] Nie można zapisać SKILL.md:",
...
app.post("/skills/:id/result", (req, res) => {
  ...
  if (typeof success !== "boolean")
    return res.status(400).json({ error: "Wymagane pole: success (boolean)" });
...
app.post("/memory/core", (req, res) => {
  ...
  if (!key?.trim() || !value?.trim())
    return res.status(400).json({ error: "Wymagane: key, value" });
...
app.get("/memory/archival", (req, res) => {
  ...
  res.json({
    entries: memory.listArchival(limit),
    count: memory.countArchival(),
  });
});

// POST /memory/archival/save — zapisz { content, source?, tags? }
app.post("/memory/archival/save", async (req, res) => {
  const { content, source, tags } = req.body as {
    content?: string;
    source?: string;
    tags?: string[];
  };
  if (!content?.trim())
    return res.status(400).json({ error: "Wymagane: content" });
  const id = await memory.saveArchival(
    content.trim(),
    source ?? "manual",
...
app.post("/memory/archival/search", async (req, res) => {
  ...
  const results = await memory.searchArchival(
    query.trim(),
    limit ?? 5,
    threshold ?? 0.55,
  );
...
app.get("/memory/recall", (req, res) => {
  ...
  const sessionId =
    typeof req.query["sessionId"] === "string"
      ...
      : undefined;
  res.json({
    entries: memory.getRecentRecall(limit, sessionId),
    count: memory.countRecall(),
  });
});
...
app.post("/memory/recall/save", (req, res) => {
  const { role, content, sessionId } = req.body as {
    role?: string;
    content?: string;
    sessionId?: string;
  };
  if (!role || !content?.trim())
    return res.status(400).json({ error: "Wymagane: role, content" });
...
app.get("/agents/list", (_req, res) => {
  const agents = loadAgents().map(({ id, name, description }) => ({
    id,
    name,
    description,
  }));
...
app.post("/agents/activate", (req, res) => {
  ...
  if (!agent)
    return res.status(404).json({ error: `Agent nie znaleziony: ${id}` });
...
app.get("/zeno/agents", async (_req, res) => {
  try {
    ...
    if (!r.ok)
      return res.status(r.status).json({ error: `Admin API ${r.status}` });
...
app.post("/zeno/agents/deploy", async (req, res) => {
  const { name, test_result, score } = req.body as {
    name?: string;
    test_result?: string;
    score?: number;
  };
  ...
  try {
    ...
    if (!r.ok)
      return res
        .status(r.status)
        .json({ error: `Admin API fetch ${r.status}` });
    ...
    if (idx < 0)
      return res.status(404).json({ error: `Agent "${name}" nie znaleziony` });
    ...
    const saveR = await fetch(ZENO_ADMIN_URL, {
      method: "POST",
      headers: {
        Authorization: `Basic ${zenoAdminToken()}`,
    ...
    if (!saveR.ok)
      return res
        .status(saveR.status)
        .json({ error: `Admin API save ${saveR.status}` });
...
const GOOSE_DESKTOP_PATH =
  process.env.GOOSE_DESKTOP_PATH ?? "U:\\Goose-1.29.1\\dist-windows\\Goose.exe";
...
app.post("/mybonzo/chat", async (req, res) => {
  ...
  const project =
    MYBONZO_PROJECTS.find((p) => p.id === projectId) ?? MYBONZO_PROJECTS[0];
  ...
  const coreSection = memory.formatCoreForPrompt();
  const systemContent =
    String(SYSTEM_PROMPT.content) + projectContext + coreSection;
...
app.get("/mybonzo/skills", (req, res) => {
  ...
  const project = projectId
    ? MYBONZO_PROJECTS.find((p) => p.id === projectId)
    : undefined;
  const ns =
...
app.get("/projects/list", async (_req, res) => {
  ...
  if (!cfConfigured()) {
    return res.json({
      projects: local,
      cfError: "CF_API_TOKEN / CF_ACCOUNT_ID brak w .env",
      timestamp: new Date().toISOString(),
    });
  }
        ...
        cf: {
          ...
          latestDeploy: cf.latest_deployment
            ? {
                ...
                branch:
                  cf.latest_deployment.deployment_trigger?.metadata?.branch,
                commitMsg:
                  cf.latest_deployment.deployment_trigger?.metadata
                    ?.commit_message,
              }
            : null,
        },
...
app.get("/projects/:id/deployments", async (req, res) => {
  ...
  if (!project)
    return res
      .status(404)
      .json({ error: `Projekt "${req.params["id"]}" nie znaleziony` });
  if (!project.cfProject)
    return res
      .status(400)
      .json({ error: "Projekt nie jest na CF Pages (Vercel?)" });
  if (!cfConfigured())
    return res
      .status(400)
      .json({ error: "CF_API_TOKEN / CF_ACCOUNT_ID brak w .env" });
...
app.get("/projects/:id/logs", async (req, res) => {
  ...
  if (!project)
    return res
      ...
      .json({ error: `Projekt "${req.params["id"]}" nie znaleziony` });
  if (!project.cfProject)
    return res.status(400).json({ error: "Projekt nie jest na CF Pages" });
  if (!cfConfigured())
    return res
      .status(400)
      .json({ error: "CF_API_TOKEN / CF_ACCOUNT_ID brak w .env" });
...
app.post("/projects/:id/deploy", async (req, res) => {
  ...
  const via =
    (req.body as { via?: "cf" | "gh" })?.via ??
    (project.cfProject ? "cf" : "gh");

  try {
    if (via === "cf") {
      if (!project.cfProject)
        return res.status(400).json({ error: "Brak cfProject — użyj via:gh" });
      if (!cfConfigured())
        return res
          .status(400)
          .json({ error: "CF_API_TOKEN / CF_ACCOUNT_ID brak w .env" });
    ...
    if (via === "gh") {
      if (!project.ghOwner || !project.ghRepo || !project.ghWorkflow) {
        return res
          .status(400)
          .json({ error: "Brak ghOwner/ghRepo/ghWorkflow w projekcie" });
      }
      if (!ghConfigured())
        return res.status(400).json({ error: "GITHUB_TOKEN brak w .env" });
      await triggerGHWorkflow(
        project.ghOwner,
        project.ghRepo,
        project.ghWorkflow,
      );
      return res.json({
        triggered: true,
        via: "gh",
        repo: `${project.ghOwner}/${project.ghRepo}`,
        workflow: project.ghWorkflow,
      });
    }
...
app.get("/projects/:id/gh/runs", async (req, res) => {
  ...
  if (!project.ghOwner || !project.ghRepo || !project.ghWorkflow) {
    return res
      .status(400)
      .json({ error: "Projekt bez konfiguracji GH Actions" });
  }
  if (!ghConfigured())
    return res.status(400).json({ error: "GITHUB_TOKEN brak w .env" });
...
app.get("/projects/:id/d1", async (req, res) => {
  ...
  if (!project.d1Id)
    return res
      .status(400)
      .json({ error: "Projekt nie ma skonfigurowanej bazy D1" });
  if (!cfConfigured())
    return res
      .status(400)
      .json({ error: "CF_API_TOKEN / CF_ACCOUNT_ID brak w .env" });
...
app.post("/projects/:id/d1/query", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(403)
      .json({
        error: "Destruktywne zapytania zablokowane. Użyj wrangler CLI.",
      });
  }
...
httpServer.listen(PORT, "127.0.0.1", () => {
  ...
  console.log(
    `Goose dostępny: ${goose.isAvailable() ? "TAK ✓" : "NIE ✗ — sprawdź .env"}`,
  );
  console.log("");
