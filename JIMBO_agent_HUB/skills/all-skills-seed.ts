...
function loadSkill(relPath: string): string {
  ...
  if (!existsSync(full)) {
    console.warn(`  ⚠  Plik nie istnieje: ${relPath}`);
    return "";
  }
...
const ALL_SKILLS = [
  ...
  // ── ANALYST (Super Analityk — wywiad wizerunkowy) ────────────────────────
  {
    name: "analyst-company-image",
    description:
      "Super Analityk — archeologia danych, wywiad wizerunkowy firmy, synteza strategiczna. Format: ZNALEZISKA → WNIOSKI → REKOMENDACJE.",
    namespace: "analyst",
    tags: [
      "analyst",
      "intelligence",
    ...
    code: loadSkill("library/analyst/company-analyst.ts"),
  },
...
async function seed() {
  console.log(
    "[all-skills-seed] Seedowanie wszystkich skills użycia narzędzi...\n",
  );
