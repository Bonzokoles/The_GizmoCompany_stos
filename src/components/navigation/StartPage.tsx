...
const SEARCH_ENGINES: SearchEngine[] = [
  {
    ...
    label: "SearXNG",
    icon: "◉",
    buildUrl: (q) => `http://localhost:8888/search?q=${encodeURIComponent(q)}`,
    hint: "Prywatne · lokalne · bez śledzenia",
  },
  {
    id: "duckduckgo",
    label: "DuckDuckGo",
  ...
  },
  {
    ...
    hint: "Prywatne · niezależny indeks",
  },
  {
    id: "kaggle",
    label: "Kaggle",
    icon: "◆",
    buildUrl: (q) => `https://www.kaggle.com/search?q=${encodeURIComponent(q)}`,
    hint: "Datasety · notebooki · modele ML",
  },
  {
    id: "perplexity",
    label: "Perplexity",
    ...
    hint: "AI-powered · podsumowania · źródła",
  },
];
...
function GlanceClockTime() {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      >
        {h}:{m}
        <span style={{ color: "#00d4ff66", fontSize: 24 }}>:{s}</span>
      </div>
    </div>
  );
}
...
function GlanceClockDate() {
  ...
  const dayOfYear = Math.ceil(
    (time.getTime() - new Date(time.getFullYear(), 0, 1).getTime()) /
      86_400_000,
  );
  return (
    <div style={{ textAlign: "center" }}>
...
const TOOL_HIGHLIGHTS = [
  {
    icon: "◇",
    name: "Glance",
    desc: "Dashboard & monitoring",
    url: "https://github.com/glanceapp/glance",
  },
  {
    icon: "≡",
    name: "daily.dev",
    desc: "Developer news feed",
    url: "https://daily.dev",
  },
  {
    icon: "⊛",
  ...
  },
  {
    icon: "▣",
    name: "Weaviate",
    desc: "Hybrid vector DB",
    url: "https://weaviate.io",
  },
  {
    icon: "▸",
    name: "txtai",
    desc: "RAG pipeline AI",
...
/* ── Center graphic ────────────────────────────────── */
function ZenoCenterGraphic() {
  const C = "#00d4ff";
  const cx = 560;
  const cy = 340;
  // radii for concentric rings
  const rings = [35, 70, 115, 170, 240, 320, 410];
  const spokes16 = Array.from({ length: 16 }, (_, i) => (i * 360) / 16);
  const spokes8 = Array.from({ length: 8 }, (_, i) => (i * 360) / 8);
  ...
  return (
    ...
    >
      <defs>
        {/* Radial fade — anchored to new center, shifted right-bottom */}
        <radialGradient id="rg" cx="70%" cy="68%" r="65%">
          <stop offset="0%" stopColor={C} stopOpacity="0.18" />
          <stop offset="40%" stopColor={C} stopOpacity="0.10" />
          <stop offset="75%" stopColor={C} stopOpacity="0.04" />
          <stop offset="100%" stopColor={C} stopOpacity="0" />
        </radialGradient>
        {/* Center glow — also shifted to new cx/cy */}
        <radialGradient id="glow" cx="70%" cy="68%" r="50%">
          <stop offset="0%" stopColor={C} stopOpacity="0.55" />
          <stop offset="35%" stopColor={C} stopOpacity="0.15" />
          <stop offset="100%" stopColor={C} stopOpacity="0" />
        </radialGradient>
        {/* Wide diffuse light — covers główna strona (left/center area) */}
        <radialGradient id="diffuse" cx="35%" cy="40%" r="70%">
      ...
      {/* Background fill — very faint cyan tint, centered right-bottom */}
      <rect x="0" y="0" width="800" height="500" fill="url(#rg)" />
      {/* Diffuse layer — spreads warm glow across main (left) side */}
      <rect x="0" y="0" width="800" height="500" fill="url(#diffuse)" />

      {/* Grid — horizontal lines */}
      {Array.from({ length: 19 }, (_, i) => i * 28).map((y, i) => (
        <line
          key={`gh${i}`}
          x1="0"
          y1={y}
      ...
      {/* Grid — vertical lines */}
      {Array.from({ length: 29 }, (_, i) => i * 28).map((x, i) => (
        <line
          key={`gv${i}`}
          x1={x}
          y1="0"
      ...
      {/* Concentric rings — outer ones faint, inner bright */}
      {rings.map((r, i) => {
        const opacity = [0.09, 0.11, 0.18, 0.28, 0.18, 0.1, 0.06][i];
        const sw = [0.4, 0.5, 0.7, 1.0, 0.6, 0.4, 0.3][i];
        const dash = i === 5 ? "5 9" : i === 6 ? "2 12" : undefined;
        return (
          <circle
            key={`ring${i}`}
            cx={cx}
      ...
      {/* 16 thin radial spokes — full length to edge of SVG */}
      {spokes16.map((deg, i) => {
        const [x1, y1] = polar(deg, rings[2]);
        const [x2, y2] = polar(deg, 520);
        return (
          <line
            key={`s16_${i}`}
            x1={x1}
      ...
      {/* 8 bold radial spokes — inner ring to mid ring, brighter */}
      {spokes8.map((deg, i) => {
        const [x1, y1] = polar(deg, rings[0]);
        const [x2, y2] = polar(deg, rings[3]);
        return (
          <line
            key={`s8_${i}`}
            x1={x1}
      ...
      {/* Arc segments — decorative arcs between ring[2] and ring[3] */}
      {spokes8.map((deg, i) => {
        const start = deg - 10;
        const end = deg + 10;
        const r1 = rings[2];
        const r2 = rings[3];
        const [ax, ay] = polar(start, r2);
        const [bx, by] = polar(end, r2);
        const [cx2, cy2] = polar(end, r1);
        const [dx, dy] = polar(start, r1);
        return (
          <path
            key={`arc${i}`}
      ...
      {/* Outer node ring (ring[3]) — 8 bright nodes */}
      {spokes8.map((deg, i) => {
        const [x, y] = polar(deg, rings[3]);
        return (
          <g key={`n3_${i}`}>
            <circle cx={x} cy={y} r="5" fill={C} fillOpacity="0.55" />
            <circle
              cx={x}
              cy={y}
              r="8"
      ...
      {/* Mid node ring (ring[1]) — 16 small dots */}
      {spokes16.map((deg, i) => {
        const [x, y] = polar(deg, rings[1]);
        return (
          <circle
            key={`n1_${i}`}
            cx={x}
            ...
            r="2.5"
            fill={C}
            fillOpacity={i % 2 === 0 ? 0.5 : 0.25}
          />
        );
      })}

      {/* Tiny tick marks on outer dashed ring */}
      {Array.from({ length: 24 }, (_, i) => i * 15).map((deg, i) => {
        const [x1, y1] = polar(deg, rings[4] - 5);
        const [x2, y2] = polar(deg, rings[4] + 5);
        return (
          <line
            key={`tick${i}`}
            x1={x1}
      ...
      {(
        ...
        return (
          <path
            key={`br${i}`}
            d={`M${x} ${y + dy * 28} L${x} ${y} L${x + dx * 28} ${y}`}
            stroke={C}
            strokeWidth="0.8"
            strokeOpacity="0.35"
          />
      ...
      {/* Horizontal & vertical HUD lines — full canvas */}
      <line
        x1="0"
        y1={cy}
        x2="800"
      ...
      {/* Small dash separators on HUD lines */}
      {[-200, -120, -60, 60, 120, 200].map((offset) => (
        <g key={`hud${offset}`}>
          <line
            x1={cx + offset}
            y1={cy - 4}
      ...
      {/* Center glow burst — larger radius for diffused look */}
    ...
    <div style={S.root}>
      ...
      <div style={S.content}>
        ...
        {openWidget && (
          <div
            style={{
              ...S.widgetContent,
              animation: "spWidgetEnter 150ms ease-out",
            }}
          >
        ...
        <div style={S.searchWrap}>
          <div style={S.searchBar}>
            ...
            <button
              ...
              title={
                includeLocal
                  ? "Lokalne zasoby: WŁĄCZONE"
                  : "Lokalne zasoby: wyłączone"
              }
            >
              ≡
            </button>
            {searchQuery && (
              <button style={S.searchGo} onClick={handleSearch}>
                ↵
              </button>
            )}
          </div>
            ...
            {SEARCH_ENGINES.map((eng) => (
              <button
                ...
                style={{
                  ...S.engineChip,
                  background:
                    searchEngine === eng.id ? "#0c0c22" : "transparent",
                  color: searchEngine === eng.id ? "#c0c0d8" : "#445",
                  borderColor:
                    searchEngine === eng.id ? "#00d4ff44" : "transparent",
                }}
                title={eng.hint}
              >
          ...
          <span style={S.searchHint}>
            {
              (
                SEARCH_ENGINES.find((e) => e.id === searchEngine) ??
                SEARCH_ENGINES[0]
              ).hint
            }
            {includeLocal && (
              <span style={{ color: "#00d4ff88" }}> + biblioteka lokalna</span>
            )}
          </span>
        </div>
        ...
        >
          <button
            ...
            onMouseLeave={(e) => {
              ...
              (e.currentTarget as HTMLElement).style.color = "#7878aa";
            }}
          >
            ⊞ Wszystkie narzędzia →
          </button>
      ...
      {showAllTools && (
        <div
          style={{
            ...S.allToolsBackdrop,
            animation: "spOverlayEnter 150ms ease-out",
          }}
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowAllTools(false);
          }}
        >
          <div
            style={{
              ...S.allToolsPanel,
              animation: "spPanelEnter 200ms cubic-bezier(0.23, 1, 0.32, 1)",
            }}
          >
            ...
            <div style={S.allToolsBody}>
              ...
              {CATEGORIES.map((cat) => (
                <div key={cat.id} style={{ marginBottom: 16 }}>
                  <h3 style={{ ...S.allToolsCatTitle, color: cat.color }}>
                    {cat.title}
                  </h3>
                  <div style={S.allToolsChips}>
                    {cat.items.map((item) => (
...
const S: Record<string, CSSProperties> = {
  ...
  inlineItemBtn: {
    ...
    cursor: "pointer",
    transition:
      "background 150ms ease-out, border-color 150ms ease-out, color 150ms ease-out, transform 150ms ease-out",
  ...
  catBtn: {
    ...
    cursor: "pointer",
    transition:
      "background 150ms ease-out, color 150ms ease-out, transform 150ms ease-out",
    whiteSpace: "nowrap" as const,
  },
  ...
  allToolsBtn: {
    ...
    cursor: "pointer",
    transition:
      "border-color 150ms ease-out, color 150ms ease-out, transform 150ms ease-out",
    fontFamily: "inherit",
    letterSpacing: 0.3,
  },
