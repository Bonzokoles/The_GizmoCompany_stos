  ...
  return (
    <div style={panelStyle}>
      ...
      <div style={resultsStyle}>
        {activeTab === "web" && (
          <WebResults results={webResults} onNavigate={onNavigate} />
        )}
        {activeTab === "deep" && <DeepResults report={deepReport} />}
        {activeTab === "local" && <LocalResults results={localResults} />}
        {activeTab === "compare" && (
          <CompareResults result={comparison} onNavigate={onNavigate} />
        )}
      </div>
    </div>
  );
...
function WebResults({
  ...
  if (!results.length)
    return (
      <div style={emptyStyle}>Wyszukaj w internecie za pomocą SearXNG</div>
    );
  return (
    <div>
      {results.map((r, i) => (
  ...
  if (!report)
    return (
      <div style={emptyStyle}>
        Deep Search: AI analizuje wyniki SearXNG i generuje raport
      </div>
    );
  return (
    <div>
      <div style={{ ...resultCardStyle, borderLeft: "3px solid #64ffda" }}>
        ...
        <p style={{ margin: 0, color: "#a8b2d1", lineHeight: 1.5 }}>
          {report.summary}
        </p>
      </div>
      ...
      {report.keyFindings.length > 0 && (
        <div style={resultCardStyle}>
          <h3 style={{ margin: "0 0 8px", color: "#ccd6f6" }}>
            🔑 Kluczowe ustalenia
          </h3>
          <ul style={{ margin: 0, paddingLeft: "20px" }}>
            {report.keyFindings.map((f, i) => (
              <li key={i} style={{ color: "#a8b2d1", marginBottom: "4px" }}>
                {f}
              </li>
            ))}
          </ul>
        ...
        <div style={resultCardStyle}>
          ...
          {report.sources.map((s, i) => (
            <div key={i} style={{ marginBottom: "6px" }}>
              ...
              <div style={{ fontSize: "12px", color: "#a8b2d1" }}>
                {s.relevance}
              </div>
            </div>
          ))}
...
function LocalResults({ results }: { results: LocalSearchResult[] }) {
  if (!results.length)
    return (
      <div style={emptyStyle}>
        Wyszukaj w lokalnych bibliotekach (SQLite FTS5)
      </div>
    );
  return (
    <div>
...
function CompareResults({
  ...
  if (!result)
    return (
      <div style={emptyStyle}>Porównaj wyniki online z lokalnymi danymi</div>
    );
  return (
    <div>
      <div style={{ ...resultCardStyle, borderLeft: "3px solid #ff6b6b" }}>
        ...
        {result.model && (
          <div style={{ ...metaStyle, marginTop: "6px" }}>◆ {result.model}</div>
        )}
      </div>

      <div style={{ display: "flex", gap: "8px" }}>
        <div style={{ flex: 1 }}>
          <h4 style={{ color: "#64ffda", margin: "8px 0 4px" }}>
            🌐 Online ({result.webResults.length})
          </h4>
        ...
        <div style={{ flex: 1 }}>
          <h4 style={{ color: "#ff6b6b", margin: "8px 0 4px" }}>
            📚 Lokalne ({result.localResults.length})
          </h4>
          {result.localResults.slice(0, 5).map((r, i) => (
            <div key={i} style={{ ...resultCardStyle, padding: "6px 8px" }}>
              <div style={{ color: "#ccd6f6", fontWeight: "bold" }}>
                {r.fileName}
              </div>
              <div
                style={{ fontSize: "11px", color: "#8892b0" }}
                dangerouslySetInnerHTML={{
                  __html: r.snippet?.substring(0, 100),
                }}
              />
            </div>
          ))}
