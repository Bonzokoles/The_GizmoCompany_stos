...
const QUICK_ACTIONS = [
  {
    icon: "🐛",
    label: "Fix błąd",
    prompt: (code: string, sel: string) =>
      `Fix the error or bug in this ${sel ? "selected" : ""} code:\n\`\`\`\n${sel || code.slice(-2000)}\n\`\`\``,
  },
  {
    icon: "📖",
    label: "Wyjaśnij",
  ...
  },
  {
    ...
    label: "Refaktor",
    prompt: (code: string, sel: string) =>
      `Refactor and improve this code (cleaner, more efficient, better naming):\n\`\`\`\n${sel || code.slice(-2000)}\n\`\`\``,
  },
  {
    icon: "✅",
    label: "Testy",
    prompt: (code: string, sel: string) =>
      `Write comprehensive unit tests (vitest/jest) for this code:\n\`\`\`\n${sel || code.slice(-2000)}\n\`\`\``,
  },
  {
    icon: "📝",
    label: "Dokumentacja",
    prompt: (code: string, sel: string) =>
      `Add JSDoc/docstring documentation to this code:\n\`\`\`\n${sel || code.slice(-2000)}\n\`\`\``,
  },
  {
    icon: "🔍",
    label: "Code Review",
    prompt: (code: string, sel: string) =>
      `Do a thorough code review of this code. Point out issues, potential bugs, and improvements:\n\`\`\`\n${sel || code.slice(-2000)}\n\`\`\``,
  },
];
      ...
      try {
        ...
        if (!accumulated) {
          const json = (await res
            .json()
            .catch(() => ({ message: "Brak odpowiedzi" }))) as {
            message?: string;
          };
          setMessages((prev) => {
            const copy = [...prev];
            copy[copy.length - 1] = {
              role: "assistant",
              content: json.message ?? "Brak odpowiedzi",
            };
            return copy;
          });
        }
      ...
      >
        {messages.map((msg, i) => (
          <div key={i} style={{ marginBottom: "10px" }}>
            ...
            >
              {msg.loading ? (
                <span style={{ color: "#7c3aed" }}>⬤ generuję...</span>
              ) : (
                msg.content
              )}
            </div>
...
export function CodeEditorPanel({ onClose }: CodeEditorPanelProps) {
  ...
  useEffect(() => {
    ...
    completionDisposableRef.current = registerCompletions(
      monaco,
      activeTab.language,
    );
    return () => {
  ...
  const handleOpen = useCallback(async () => {
    ...
    if (!api) {
      setStatus("Tylko Electron — brak dialog API");
      return;
    }
    try {
      // Use dialog API
    ...
    >
      ...
      >
        ...
        <button
          onClick={handleOpen}
          style={btnStyle}
          title="Otwórz plik (Ctrl+O)"
        >
          📁 Otwórz
        </button>
        <button onClick={handleSave} style={btnStyle} title="Zapisz (Ctrl+S)">
          💾 Zapisz
        </button>
        <button onClick={handleNewTab} style={btnStyle} title="Nowy plik">
          📄 Nowy
        </button>
        <div style={{ flex: 1 }} />
        <span
        ...
        <button
          ...
          style={{
            ...btnStyle,
            background: showAi ? "#7c3aed" : "#1e293b",
            color: "#fff",
          }}
