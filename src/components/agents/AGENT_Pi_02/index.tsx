 ...
 * Używany w: AgentWorkspacePanel.tsx → slot TERMINAL_SLOT_02
 */
...
export interface AgentTerminal_02_Props {
  agentDef: AgentDef;
  workspaceDir: string;
  onRemove: () => void;
...
export function AgentTerminal_02({
  agentDef,
  workspaceDir,
  onRemove,
  ref,
}: AgentTerminal_02_Props) {
  ...
  useEffect(() => {
    ...
    if (isElectron) {
      term.writeln(
        `\x1b[1m${agentDef.icon} ${agentDef.name.toUpperCase()} [SLOT 02]\x1b[0m`,
      );
  ...
  const launchWithParams = useCallback(
    async (provider: string, model: string) => {
      ...
      term.writeln(`\r\n\x1b[32mStarting ${agentDef.name} [SLOT 02]...\x1b[0m`);
      term.writeln(
