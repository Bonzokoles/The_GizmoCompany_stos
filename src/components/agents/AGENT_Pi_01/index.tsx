/**
 * AGENT_Pi_01 — Terminal slot 01 (Gemini CLI)
 * ─────────────────────────────────────────────────────────────────────────────
 * Dedykowany terminal PTY dla Gemini CLI.
 * Funkcje i modele dopiszesz tutaj indywidualnie.
 *
 * Eksportuje: AgentTerminal_01
 * Używany w: AgentWorkspacePanel.tsx → slot TERMINAL_SLOT_01
 */
...
// ── Handle imperativo — Pi może wywołać metody tego terminala z zewnątrz ────
export interface SlotHandle {
  /** Wyślij tekst bezpośrednio do PTY (jak wpisanie z klawiatury) */
  sendInput: (text: string) => void;
  /** Uruchom agenta z konkretnym dostawcą i modelem */
  launchWith: (provider: string, model: string) => void;
  /** Zatrzymaj sesję PTY */
  kill: () => void;
  /** Czy agent aktualnie działa */
  isRunning: () => boolean;
  /** Ustaw dostawcę i model bez uruchamiania */
  setProviderModel: (provider: string, model: string) => void;
}

export interface AgentTerminal_01_Props {
  agentDef: AgentDef;
  workspaceDir: string;
  onRemove: () => void;
  /** React 19: ref jako prop — Pi może przejąć kontrolę nad tym terminalem */
...
export function AgentTerminal_01({
  agentDef,
  workspaceDir,
  onRemove,
  ref,
}: AgentTerminal_01_Props) {
  ...
  const [selectedModel, setSelectedModel] = useState(
    agentDef.defaultModel ||
      ...
      "gemini-1.5-flash", // Zmieniono na gemini-1.5-flash
  );

  // Synchronizuj model przy zmianie dostawcy
  useEffect(() => {
    setSelectedModel(
      PROVIDER_DEFAULTS[selectedProvider]?.model ?? "gemini-1.5-flash", // Zmieniono na gemini-1.5-flash
    );
  }, [selectedProvider]);
  ...
  useEffect(() => {
    ...
    if (isElectron) {
      term.writeln(
        `\x1b[1m✦ ${agentDef.name.toUpperCase()} [SLOT 01]\x1b[0m`, // Zmieniono ikonę na ✦
      );
  ...
  // launchWithParams: core logic z jawnym provider/model → wywoływany też przez Pi Control
  ...
  const launchWithParams = useCallback(
    async (provider: string, model: string) => {
      ...
      term.writeln(`\r\n\x1b[32mStarting ${agentDef.name} [SLOT 01]...\x1b[0m`);
      term.writeln(
  ...
  // Publiczny launch — używa aktualnych wartości state
